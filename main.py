...
from src.utils import count_acc, Averager, csv_write, square_euclidean_metric, square_euclidean_metric2
...
def Test_phase(model, args, k):
    ...
    for i in range(TEST_SIZE):
        for episode in test_loader:
            ...
            emb_data_query = model(data_query)
            emb_data_shot = model(data_shot)
            emb_data_shot = emb_data_shot.view(args.nway, args.kshot,-1)
            emb_data_shot = torch.mean(emb_data_shot,1)
            
            logit = square_euclidean_metric2(emb_data_query, emb_data_shot)
            #logit = logit.view(-1,args.nway, args.kshot)
            #logit = torch.sum(logit,1)
            del(emb_data_shot)
            del(emb_data_query)
...
def train(args):
    ...
    lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.99)
    ...
    for i in range(TOTAL):
        for episode in data_loader:
            ...
            proto = None; logit = None; loss = None
