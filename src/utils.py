...
def square_euclidean_metric(a, b):
    ...
    logits = torch.pow(a - b, 2).mean(2)

    return logits

def square_euclidean_metric2(a, b):
    """ Measure the euclidean distance (optional)
    Args:
        a : torch.tensor, features of data query
        b : torch.tensor, mean features of data shots or embedding features
    ...
    logits = -1 * torch.pow(a - b, 2).sum(2)

    return logits


def cosine_metric(a, b):
    """ Measure the cosine distance (optional)
    Args:
        a : torch.tensor, features of data query
        b : torch.tensor, mean features of data shots or embedding features
...
class csv_write():
    ...
    def __init__(self, args):
        self.f = open('20202861_junseongkim.csv', 'w', newline='')
        self.write_number = 1
